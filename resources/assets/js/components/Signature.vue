...
export default {
	data() {
		return {
			signatures: [],
			pageCount: 1,
			endpoint: location.protocol+'//'+location.host+location.pathname + '/api/signatures?page='
		}
	},
	created() {
		this.fetch();
	...
	methods: {
		...
		report(id) {
			if(confirm('Are you sure you want to report this signature?')) {
				axios.put(location.protocol+'//'+location.host+location.pathname + '/api/signatures/' + id + '/report')
